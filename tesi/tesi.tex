...
stabilizzare reti di code \cite{tassiulas1992stability}. Applicato al traffico, confronta la quantità di
...
Il migliore scenario è \texttt{mp\_nmin\_a080\_f2} in low e medium, mentre in
...
un controllore locale non conosce.
...
un baseline debole. Al contrario, si è dimostrato competitivo soprattutto con
...
controllore che completa meno veicoli può apparire migliore su attesa media o
tempo di viaggio.
...
La rete Bologna è stata utilizzata tramite i file presenti in
...
Il baseline \texttt{fixed\_program0} forza il programma semaforico 0 già
...
forte di quanto ci si aspetterebbe da un semplice piano statico. La ragione è
...
Il risultato pratico è che \texttt{mp\_base} rimane una baseline fondamentale
...
controllore cambia spesso per rispondere a piccole differenze di pressione può
...
vero che un controllore dinamico debba essere sempre più veloce di uno fisso.
Se il traffico non giustifica lo switch, la decisione migliore può essere
proprio non comportarsi da Max-Pressure. Da qui nasce \texttt{mp\_program0}.
...
andare bene in balanced perché la pressione è distribuita, ma fallire in skewed
...
studio di un controllore ibrido e regime-aware.
...
La valutazione su Bologna ha un ruolo diverso rispetto a Manhattan. Qui non si
vuole soltanto verificare se Max-Pressure funzioni, ma capire come si comporti
...
Per questo i numeri vanno letti come base interpretativa: servono a spiegare
...
\begin{figure}[htbp]
    ...
    \caption{Esempio di accumulo di veicoli in una simulazione Bologna con
...
incroci molto vicini. In questi casi una metrica sola non è sufficiente. Il
...
\texttt{mp\_super}. La scelta di mostrare questi quattro casi rende leggibile
...
zona in cui il cutoff influenza maggiormente la lettura, quindi la valutazione
...
La campagna senza cutoff serve proprio a distinguere un vero miglioramento da
un effetto dovuto ai viaggi rimasti incompleti.

\section{Uso delle modalità di mp\_super}

Il comportamento del super è interpretabile perché ogni step viene assegnato a
...
omogenei. Su Bologna, invece, \texttt{fixed\_program0} è un baseline forte: è il
...
sempre, ma come strumenti utili in regimi specifici. LTA è più sensato quando
...
\begin{enumerate}
    ...
    \item Su Bologna, \texttt{fixed\_program0} è un baseline forte. Questo ha
