...
Il throughput osservato coincide con il numero di viaggi completati. Quando la simulazione
...
\texttt{bologna.net.xml} contiene 8439 junction, 10281 edge stradali,
10896 lane e 228 junction semaforizzate. Per ogni semaforo sono presenti due
logiche, per un totale di 456 \texttt{tlLogic} e 2216 fasi.
...
statico della mappa può incorporare implicitamente scelte di coordinamento che
un controller locale non conosce.
...
viaggi completati, viaggi non completati, censura, eventi di spillback, step di
blocco downstream, step di estensione platoon e contatori interni del
...
veicoli verso una lane downstream già piena, l'effetto locale può essere
...
supera una soglia critica. La campagna finale in corso include anche
\texttt{mp\_super\_no\_safe}, variante identica al super ma senza ramo safe, in
modo da misurare separatamente il costo e il beneficio di questa protezione.
...
\texttt{--max-steps 0}, cioè senza limite anticipato. I risultati riportati in
questo capitolo sono provvisori e provengono dalla campagna esplorativa
\texttt{run\_0024\_20260527\_122000}, eseguita con un seed e cutoff a 7000
step. I numeri saranno sostituiti con quelli della campagna finale a tre seed,
che include anche \texttt{mp\_super\_no\_safe}. Le conclusioni qualitative
rimangono comunque utili perché descrivono il percorso progettuale seguito.
...
forte di quanto ci si potesse aspettare da un semplice piano statico. La ragione è
che il programma nativo non è una sequenza artificiale uguale per tutti gli
incroci, ma una configurazione già associata alla rete reale.
...
risultati mostrano che questa uniformazione spesso peggiora rispetto al
\texttt{program0} originale. Questo rafforza l'interpretazione secondo cui il
piano statico della mappa contiene informazioni utili sulla rete e non va
considerato un riferimento banale.
...
In pratica, \texttt{mp\_base} rimane una baseline fondamentale
ma non è sufficiente per Bologna. Nella campagna esplorativa, ad esempio, su
domanda balanced low l'attesa media di \texttt{mp\_base} è 431 secondi contro
203 secondi di \texttt{fixed\_program0}; in balanced medium è 635 contro 529.
Il Max-Pressure puro recupera terreno solo in alcuni casi high, ma con censura
alta e quindi con interpretazione più delicata.
...
non richiede un'ottimizzazione globale della rete, ma diventa un limite quando
il piano statico ha una forma di coordinamento implicita. Bologna contiene
corridoi e successioni di incroci ravvicinati; una decisione locale aggressiva
può migliorare una coda e peggiorare quella successiva.
...
interpretare con run brevi. Il livello high non rappresenta una condizione
...
in modo uniforme. \texttt{Skewed} mantiene partenze uniformi ma campiona più
spesso route associate ad archi importanti, creando corridoi più caricati.
\texttt{Peak} usa lo stesso campionamento spaziale di skewed e aggiunge un
profilo temporale concentrato, con parametro di picco 0,8.
...
La modalità deve inoltre essere confermata per quattro step consecutivi. Questa
...
subito a \texttt{program0} appena il load scende leggermente.

Nelle campagne esplorative \texttt{mp\_program0} ha confermato l'utilità di
questa idea: su balanced high è risultato il migliore tra i controller
considerati, con 711 secondi di attesa media contro 746 di
\texttt{fixed\_program0} e 784 di \texttt{mp\_super}. Su skewed medium ha
ottenuto 678 secondi contro 751 del fixed. Questi risultati mostrano che una
regola ibrida semplice può essere molto competitiva quando il piano statico è
forte.
...
marcate. Balanced high favorisce \texttt{mp\_program0}, skewed high favorisce
LTA, peak high favorisce Nmin. Il meta-controller cerca quindi di scegliere
online la famiglia più adatta al regime locale.
...
La variante \texttt{mp\_super\_no\_safe}, aggiunta nella campagna finale, usa
le stesse regole ma disabilita la selezione di \texttt{safe}. In questo modo è
possibile verificare se il ramo di protezione aiuta davvero o se introduce
prudenza eccessiva.

\subsection{Load}

Il load di \texttt{mp\_super} è calcolato come in
...
immediati a \texttt{program0} quando il carico cala per pochi step.
...
La burstiness misura quanto cambia la domanda fra lo step corrente e quello
...
delle lane downstream servite dalla fase. Il ramo \texttt{safe} richiede
downstream score almeno 0,97, load almeno 0,72 e presenza di burstiness o wait
imbalance. Quando si attiva, il controller evita fasi che manderebbero altri
veicoli verso una lane quasi satura e applica la penalità downstream. Lo scopo
...
Le regole finali sono ordinate per priorità. Prima viene valutato \texttt{safe}
se abilitato. Poi, se il carico è sotto soglia, il controller resta o torna a
\texttt{program0}. Nei regimi ad alto carico, burstiness porta a Nmin; attesa e
domanda sbilanciate portano a LTA; attesa sbilanciata senza forte imbalance
porta a fairness; un platoon compatto può portare alla modalità platoon. In
assenza di segnali specialistici viene usato \texttt{base}.
...
Oltre all'isteresi sul load, \texttt{mp\_super} usa una conferma temporale:
una nuova modalità deve essere proposta per quattro step consecutivi prima di
...
segno che la rete presenta molti regimi locali diversi e che una parte del
comportamento resta oscillatoria. La campagna finale serve anche a valutare se
le soglie più recenti rendono il router più stabile.

\section{Evoluzione delle soglie di mp\_super}

L'evoluzione di \texttt{mp\_super} è stata guidata dai risultati. Le prime
versioni usavano soglie più rigide per burstiness e platoon; di conseguenza
alcuni rami non venivano mai selezionati. Successivamente sono stati aggiunti
\texttt{wait\_imbalance}, Nmin forte e LTA come rami espliciti. Il ramo
\texttt{safe} è stato reso più raro, con soglia downstream 0,97, e infine è
stata introdotta \texttt{mp\_super\_no\_safe} per verificare se questa
protezione sia necessaria.

Il punto metodologico importante è che ogni modifica è stata motivata da una
diagnosi: spillback che non si attiva, safe troppo presente, platoon score
sempre nullo, program0 dominante in low, specialisti diversi vincenti nei casi
high. In questo senso Bologna non è soltanto una seconda mappa di test, ma il
caso che ha trasformato il progetto da confronto fra varianti Max-Pressure a
studio di un controller ibrido e regime-aware.
...
La valutazione su Bologna ha un ruolo diverso rispetto a Manhattan. In questo
caso non si vuole soltanto verificare se Max-Pressure funzioni, ma capire come si comporti
quando compete con un programma semaforico reale già presente nella mappa. Il
confronto usa tre livelli di domanda, tre distribuzioni spaziali e temporali
(balanced, skewed e peak) e gli scenari principali della pipeline: programma
fisso, Max-Pressure base, \texttt{mp\_program0}, \texttt{mp\_super} e gli
specialisti LTA, Nmin, spillback, fairness e platoon.

I risultati numerici riportati in questo capitolo provengono dalla campagna
esplorativa \path{run_0024_20260527_122000}, con un seed e cutoff a 7000 step.
La campagna finale predisposta nel software usa invece \path{bologna_thesis_v1},
tre seed, nessun cutoff anticipato e include anche \texttt{mp\_super\_no\_safe}.
Per questo i numeri vanno letti come base interpretativa: aiutano a spiegare
l'evoluzione del metodo e saranno sostituiti dai risultati definitivi quando la
campagna senza censura sarà completata.
...
La \cref{fig:bologna-congestion} mostra il tipo di condizione che non appare
nella griglia Manhattan: code concentrate su corridoi specifici, archi corti e
incroci molto vicini. In questi casi una singola metrica non è sufficiente. Il
tempo medio di attesa resta la misura principale, ma nei casi high deve sempre
essere letto insieme alla censura, cioè alla quota di viaggi non completati
entro il cutoff.

\section{Risultati aggregati}

La \cref{fig:bologna-wait-bars} confronta i quattro controller più importanti:
\texttt{fixed\_program0}, \texttt{mp\_base}, \texttt{mp\_program0} e
\texttt{mp\_super}. La scelta di mostrare questi quattro casi rende più chiaro
il messaggio centrale del capitolo: il Max-Pressure puro non basta, mentre le
versioni ibride recuperano il comportamento del programma nativo quando il
traffico è basso e introducono adattività quando il carico cresce.
...
\begin{figure}[htbp]
    ...
    \begin{axis}[
        ...
        grid=major,
        legend style={at={(0.5,-0.24)},anchor=north,legend columns=4,font=\scriptsize}
    ]
        \addplot coordinates {(Bal-L,203) (Bal-M,529) (Bal-H,746) (Skw-L,405) (Skw-M,751) (Skw-H,1221) (Peak-L,699) (Peak-M,999) (Peak-H,1356)};
        \addlegendentry{Fixed}
        \addplot coordinates {(Bal-L,431) (Bal-M,635) (Bal-H,1043) (Skw-L,672) (Skw-M,1191) (Skw-H,1023) (Peak-L,735) (Peak-M,1255) (Peak-H,1185)};
        \addlegendentry{MP base}
        \addplot coordinates {(Bal-L,194) (Bal-M,436) (Bal-H,711) (Skw-L,277) (Skw-M,678) (Skw-H,1204) (Peak-L,462) (Peak-M,902) (Peak-H,1432)};
        \addlegendentry{MP program0}
        \addplot coordinates {(Bal-L,190) (Bal-M,383) (Bal-H,784) (Skw-L,254) (Skw-M,704) (Skw-H,1103) (Peak-L,408) (Peak-M,893) (Peak-H,1335)};
        \addlegendentry{MP super}
    \end{axis}
    \end{tikzpicture}
    \caption{Confronto sintetico dei controller principali su Bologna.}
    \label{fig:bologna-wait-bars}
\end{figure}

Nei tre casi low il \texttt{mp\_super} è il migliore tra i controller principali:
190 secondi in balanced, 254 in skewed e 408 in peak. Questo risultato è
importante perché risponde al problema emerso durante lo sviluppo: in traffico
leggero il Max-Pressure puro cambia fase troppo facilmente e paga molti costi
di transizione; il super, invece, resta quasi sempre sul programma nativo e
interviene solo quando vede un segnale locale sufficientemente forte.

Nei casi medium il comportamento rimane favorevole: \texttt{mp\_super} vince in
balanced e peak, mentre in skewed è leggermente peggiore di
\texttt{mp\_program0}. Nei casi high il quadro è più delicato. In balanced high
vince \texttt{mp\_program0}; in skewed high il super migliora il fixed; in peak
high il super è vicino al fixed come attesa ma completa più viaggi. Questa è la
zona in cui il cutoff influenza maggiormente la lettura; per questo la valutazione
finale richiede simulazioni senza limite anticipato.

\section{Censura nei casi ad alto traffico}

La \cref{fig:bologna-censor-scatter} mostra perché i casi high non possono
essere giudicati soltanto con l'attesa media. Alcuni specialisti ottengono
attese più basse, ma al prezzo di una censura molto alta. In altre parole,
sembrano veloci anche perché lasciano fuori dal calcolo molti viaggi non
conclusi entro 7000 step.
...
\begin{figure}[htbp]
    ...
    \begin{axis}[
        ...
        \addplot+[only marks,mark=*] coordinates {(12.1,746) (25.9,1221) (28.4,1356)};
        \addlegendentry{Fixed}
        \addplot+[only marks,mark=triangle*] coordinates {(5.5,711) (20.8,1204) (21.8,1432)};
        \addlegendentry{MP program0}
        \addplot+[only marks,mark=square*] coordinates {(5.5,784) (19.5,1103) (21.6,1335)};
        \addlegendentry{MP super}
        \addplot+[only marks,mark=diamond*] coordinates {(18.8,969) (48.2,913) (51.3,1027)};
        \addlegendentry{Miglior specialista}
    \end{axis}
    \end{tikzpicture}
    \caption{Relazione tra attesa media e censura nei casi high di Bologna.}
    \label{fig:bologna-censor-scatter}
\end{figure}

Il caso peak high chiarisce il problema. Nmin ottiene 1027 secondi di attesa,
ma con 51,3\% di censura; \texttt{mp\_super} ottiene 1335 secondi, ma con
21,6\% di censura; il fixed arriva a 1356 secondi con 28,4\% di censura. Se si
considera solo l'attesa, Nmin sembra nettamente migliore. Se si considera anche
il completamento dei viaggi, il super appare più conservativo ma più robusto.
La campagna senza cutoff serve a distinguere un vero miglioramento da
un effetto dovuto ai viaggi rimasti incompleti.

\section{Uso delle modalità di mp\_super}

Il comportamento del super è interpretabile perché ogni step viene associato a
una modalità. Il dato più utile non è soltanto quale ramo venga scelto, ma
quanto spesso il controller abbandoni \texttt{program0}. La
\cref{fig:super-non-program0} mostra questa quota aggregata.
...
\begin{figure}[htbp]
    ...
    \begin{axis}[
        ...
        \addplot coordinates {(Bal-L,0.1) (Bal-M,0.7) (Bal-H,4.1) (Skw-L,0.2) (Skw-M,1.9) (Skw-H,7.6) (Peak-L,0.4) (Peak-M,2.7) (Peak-H,9.1)};
    \end{axis}
    \end{tikzpicture}
    \caption{Quota di step in cui \texttt{mp\_super} usa una modalità diversa
    da \texttt{program0}.}
    \label{fig:super-non-program0}
\end{figure}

La figura conferma il principio progettuale: in low il super si comporta quasi
come il programma nativo, mentre in high aumenta gradualmente l'uso dei rami
Max-Pressure. Le modalità specialistiche sono attivate da segnali locali:
\textit{load} misura il carico normalizzato delle corsie osservate,
\textit{imbalance} misura quanto una direzione sia più carica delle altre,
\textit{wait imbalance} considera lo squilibrio nei tempi di attesa,
\textit{burstiness} intercetta ondate compatte di veicoli e la saturazione
downstream abilita il ramo safe solo come protezione estrema. Questo rende il
controller dinamico: non sceglie la modalità in base al nome della domanda, ma
in base a ciò che ogni semaforo vede in tempo reale.

Nella campagna esplorativa \texttt{program0} domina ancora anche in high. Questo
non è necessariamente un errore: molti semafori locali restano scarichi anche
quando la rete nel suo complesso è congestionata. Tuttavia i risultati high
hanno motivato la taratura successiva, aumentando l'uso dei rami LTA e Nmin
quando il carico è alto e riducendo il ricorso al safe, che deve rimanere una
ultima risorsa.

\section{Discussione dei risultati}

La valutazione su Bologna porta a una conclusione più realistica di quella su
Manhattan. Sulla griglia sintetica il Max-Pressure vince in modo netto perché
il piano fisso non possiede coordinamento specifico e gli incroci sono quasi
omogenei. Su Bologna, invece, \texttt{fixed\_program0} è una baseline forte: è il
programma nativo della mappa, contiene durate e transizioni pensate per quella
rete e in traffico leggero evita switch inutili.

Questo spiega perché \texttt{mp\_base} sia spesso peggiore: osserva la pressione
locale e può cambiare fase anche quando il guadagno è piccolo, pagando giallo e
all-red. \texttt{mp\_program0} corregge questo difetto usando il piano fisso
come modalità di default e passando a Max-Pressure solo oltre soglia.
\texttt{mp\_super} generalizza l'idea: oltre a decidere fra fisso e adattivo,
prova a scegliere quale variante Max-Pressure usare quando il traffico lo
richiede.

Gli specialisti non vanno quindi letti come controller alternativi da usare
sempre, ma come strumenti utili in regimi specifici. LTA è più indicato quando
esiste uno squilibrio persistente; Nmin forte quando il traffico arriva a
ondate; platoon quando sono presenti gruppi compatti; safe quando la corsia a
valle è quasi satura. La difficoltà del super è selezionare questi strumenti
senza attivarli troppo presto.

\section{Limiti della valutazione}

I risultati Bologna riportati qui hanno tre limiti. Il primo è statistico: la
campagna esplorativa usa un solo seed, quindi non misura la variabilità fra
popolazioni equivalenti. Il secondo riguarda il cutoff a 7000 step, che nei
casi high produce censura e rende ambigua la classifica. Il terzo riguarda le
soglie del super: sono interpretabili e motivate dai log, ma restano scelte
manuali.

Questi limiti non indeboliscono il contributo metodologico, ma delimitano ciò
che si può concludere. Con i dati attuali si può affermare che Bologna richiede
un controllo ibrido e regime-aware; non si può ancora dichiarare una vittoria
definitiva del super in ogni regime. La campagna finale con tre seed, nessun
cutoff e variante \texttt{mp\_super\_no\_safe} è il passo necessario per
chiudere il confronto in modo quantitativamente solido.
...
semaforico adattivo basato su Max-Pressure. Il lavoro è partito dal principio
teorico della pressione, lo ha trasformato in un controller TraCI per SUMO e ha
costruito una pipeline sperimentale riproducibile: generazione delle
popolazioni, esecuzione batch, confronto fra scenari, calcolo di attesa, tempo
di viaggio, time loss, throughput e censura.

La prima parte sperimentale ha mostrato che su Manhattan \(8\times8\) il
Max-Pressure riduce in modo netto l'attesa rispetto al piano statico, con
censura nulla. La seconda parte ha mostrato che Bologna è più complessa: il
piano \texttt{program0} è forte, il Max-Pressure puro non basta e la strategia
migliore dipende dal regime di traffico. Da questa evidenza sono nati
\texttt{mp\_program0} e \texttt{mp\_super}.
...
\begin{enumerate}
    ...
    64,26\% in low, del 60,09\% in medium e del 53,05\% in high.
    \item Le estensioni specialistiche non sono automaticamente utili. Alcune
    migliorano in regimi specifici, altre rimangono inattive oppure peggiorano
    per costo di switch o eccesso di prudenza.
    \item Su Bologna, \texttt{fixed\_program0} è una baseline forte. Questo ha
    reso necessario un controller ibrido: \texttt{mp\_program0} usa il piano
    statico in low e passa a Max-Pressure solo oltre soglia.
    \item \texttt{mp\_super} generalizza l'ibridazione scegliendo online fra
    program0, base, LTA, Nmin, fairness, platoon e safe. I risultati
    provvisori mostrano miglioramenti importanti in low e medium, mentre i
    casi high richiedono la campagna finale senza censura per una conclusione
    definitiva.
\end{enumerate}
...
valutabile. L'obiettivo è stato raggiunto: il controller base e le sue varianti
sono integrati nella pipeline e producono metriche confrontabili.

Il secondo obiettivo era costruire una metodologia sperimentale riproducibile.
Anche questo obiettivo è stato raggiunto: le run sono descritte da file di
configurazione, popolazioni e report automatici. L'introduzione della censura è
stata decisiva per evitare interpretazioni fuorvianti.

Il terzo obiettivo era valutare il comportamento su reti diverse. Manhattan ha
fornito un risultato pulito a favore di Max-Pressure; Bologna ha mostrato i
limiti dell'approccio puro e ha motivato controller ibridi. La tesi risponde
quindi non con una singola classifica, ma con una conclusione più generale:
Max-Pressure è efficace, ma nelle reti reali va adattato al regime e al piano
semaforico esistente.

\section{Limiti}

Il limite principale è che i risultati Bologna presenti in questa versione sono
provvisori. La campagna finale con tre seed, nessun cutoff e
\texttt{mp\_super\_no\_safe} è necessaria per trasformare le conclusioni
qualitative in risultati definitivi. Un secondo limite è che il meta-controller
è rule-based: è interpretabile, ma le soglie sono scelte manualmente. Un terzo
limite è l'assenza di un coordinamento globale esplicito fra incroci: il
controller resta locale.

\section{Sviluppi futuri}

Gli sviluppi futuri più naturali sono cinque. Primo, completare la campagna
Bologna finale e sostituire le tabelle provvisorie con medie e deviazioni
standard su tre seed. Secondo, confrontare \texttt{mp\_super} e
\texttt{mp\_super\_no\_safe} per decidere se il ramo safe sia realmente utile.
Terzo, calibrare le soglie del super con procedure automatiche, ad esempio
grid search o ottimizzazione bayesiana, mantenendo però vincoli di
interpretabilità. Quarto, studiare un meta-controller appreso, come un decision
tree addestrato sui log delle simulazioni, da confrontare con le regole manuali.
Quinto, introdurre forme leggere di coordinamento fra incroci adiacenti o
segnali regionali, così da ridurre il rischio che decisioni locali rompano
progressioni utili. Questa direzione è coerente con lavori recenti come
Smoothing-MP e N-MP, che mantengono l'impostazione Max-Pressure ma aggiungono
informazione di coordinamento o stato di rete \cite{xu2024smoothing,liu2024nmp}.


\chapter{Appendici}
\label{chap:appendici}

Questo capitolo raccoglie il materiale tecnico necessario per replicare gli
...
5/20, platoon x2 e \texttt{mp\_super\_no\_safe}, più
\texttt{fixed\_program0} e \texttt{fixed\_tuned}.
...
\path{logs/ablation/runs/run_0023_20260612_131724}. I risultati provvisori di
Bologna usati nel Capitolo~\ref{chap:bologna-risultati} provengono da
\path{logs/ablation/runs/run_0024_20260527_122000}; la campagna finale
sostituirà questi valori appena conclusa.

\section{Struttura essenziale del software}
\label{app:software}

La repository è organizzata intorno a pochi file principali. \texttt{runner.py}
